<?php
	...
	//-----------------------------------------------------------------
		...
		if(isset($_SESSION["vercode"])){ //Check if session 'vercode' is set
			if($_POST["vercode"] != $_SESSION["vercode"] OR $_SESSION["vercode"] == ""){
				...
				unset($_SESSION["vercode"]); //Unsetting session 'vercode' when captcha is confirmed
			}	
		}	
	}
	//-----------------------------------------------------------------
	...
	elseif ($approved == 0 AND $eMail != "" AND $password != "") { //If the account is not approved yet...
		if($err == ""){//and if there is no other error...
			$err = "Your account is not approved by admin!"; //alert user and give the link to activation page
		} 
		$errFlag = 1; //Set the error flag high
	}
	//-----------------------------------------------------------------
